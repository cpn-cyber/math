...
Step31最终审计显示，第三问核心文件无缺失，数据一致性通过，核心AI风险表与论文正文保留边界声明，最终审计无FAIL；审计状态统计为 PASS={audit.get("status_counts", {}).get("PASS", "待补充")}，WARNING={audit.get("status_counts", {}).get("WARNING", "待补充")}，FAIL={audit.get("status_counts", {}).get("FAIL", 0)}。
...
{_markdown_table(current_rows, [("paper_id", "论文"), ("F1", "F1_score"), ("F2", "F2_key_score"), ("Q", "Q_cur_baseline"), ("grade", "基线相对等级（封版聚类）")])}

由表可见，3-2当前基线较强，3-3次之，3-1短板最明显。需要注意的是，表中的等级是按问题1封版聚类边界得到的基线相对等级，用于描述三篇论文在封版评价体系下的相对位置；Step29中的“预测达标等级”采用固定阈值（60合格、75良好、85优秀），二者口径不同，不应直接理解为“优秀退化为合格”。
...
{_markdown_table(prediction_rows, [("paper_id", "论文"), ("before", "修改前分"), ("after", "预测修改后分"), ("gain", "预测提升"), ("level", "预测达标等级（固定阈值）"), ("pass_ratio", "稳健达到合格比例")])}

按固定达标阈值判断，3-1优化后仍难达到合格，3-2优化后稳定保持合格，3-3有较大概率提升到合格。三篇论文AI辅助风险提示指标整体下降，多智能体分歧整体呈下降趋势。但上述结果均为模型预测值，不等同于实际人工修改后的真实评分。
...
def _tables_md(ctx: dict[str, Any]) -> str:
    rows = [
        {"title": "表X 附件3当前质量复评结果", "source": "output/problem3_tables/appendix3_current_evaluation.xlsx", "position": "6.2", "note": "展示F1_score、F2_key_score、Q_cur_baseline和封版聚类相对等级，并说明其与预测达标等级口径不同。"},
